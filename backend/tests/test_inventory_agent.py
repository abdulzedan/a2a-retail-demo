"""
Unit tests for the Inventory Agent A2A.
"""

import pytest
from unittest.mock import Mock, patch

from backend.agents.inventory_agent_a2a.agent import InventoryAgent


class TestInventoryAgent:
    """Test suite for InventoryAgent."""

    @pytest.fixture
    def inventory_agent(self, mock_vector_store, mock_agent_runner):
        """Create an InventoryAgent instance with mocked dependencies."""
        with patch("backend.agents.inventory_agent_a2a.agent.VertexSearchStore", return_value=mock_vector_store):
            with patch("backend.agents.inventory_agent_a2a.agent.Runner", return_value=mock_agent_runner):
                agent = InventoryAgent()
                agent._search_store = mock_vector_store
                agent._runner = mock_agent_runner
                return agent

    def test_agent_initialization(self, inventory_agent):
        """Test that the agent initializes correctly."""
        assert inventory_agent._agent is not None
        assert inventory_agent._agent.name == "inventory_agent"
        assert inventory_agent._agent.model == "gemini-2.0-flash"
        assert inventory_agent._search_store is not None
        assert inventory_agent._runner is not None

    def test_build_agent_tools(self, inventory_agent):
        """Test that the agent builds with the correct tools."""
        agent = inventory_agent._build_agent()

        # Check that agent has tools
        assert hasattr(agent, "tools")
        assert len(agent.tools) >= 3

        # Check tool names
        tool_names = [tool.__name__ for tool in agent.tools]
        assert "check_product_availability" in tool_names
        assert "search_products_by_query" in tool_names
        assert "search_products_by_category" in tool_names

    @pytest.mark.asyncio
    async def test_check_product_availability_success(self, inventory_agent):
        """Test checking product availability for existing product."""
        # Get the tool directly
        agent = inventory_agent._build_agent()
        check_tool = next(t for t in agent.tools if t.__name__ == "check_product_availability")

        # Execute the tool
        result = check_tool("PROD-001")

        # Verify the result
        assert result["status"] == "available"
        assert result["product"]["id"] == "PROD-001"
        assert result["product"]["name"] == "Smart TV 55-inch 4K"
        assert result["product"]["stock_quantity"] == 15
        assert result["message"] == "Product PROD-001 is available with 15 units in stock."

    @pytest.mark.asyncio
    async def test_check_product_availability_not_found(self, inventory_agent):
        """Test checking availability for non-existent product."""
        agent = inventory_agent._build_agent()
        check_tool = next(t for t in agent.tools if t.__name__ == "check_product_availability")

        result = check_tool("PROD-999")

        assert result["status"] == "not_found"
        assert result["product"] is None
        assert "not found" in result["message"]

    @pytest.mark.asyncio
    async def test_search_products_by_query(self, inventory_agent):
        """Test searching products by query."""
        agent = inventory_agent._build_agent()
        search_tool = next(t for t in agent.tools if t.__name__ == "search_products_by_query")

        result = search_tool("smart tv")

        assert result["status"] == "success"
        assert len(result["products"]) == 2
        assert result["total_found"] == 2
        assert all(product["id"] in ["PROD-001", "PROD-002"] for product in result["products"])

    @pytest.mark.asyncio
    async def test_search_products_by_category(self, inventory_agent):
        """Test searching products by category."""
        agent = inventory_agent._build_agent()
        category_tool = next(t for t in agent.tools if t.__name__ == "search_products_by_category")

        result = category_tool("Electronics")

        assert result["status"] == "success"
        assert result["category"] == "Electronics"
        assert len(result["products"]) > 0

    @pytest.mark.asyncio
    async def test_search_products_empty_results(self, inventory_agent, mock_vector_store):
        """Test searching products with no results."""
        # Override the mock to return empty results
        mock_vector_store.search.return_value = []

        agent = inventory_agent._build_agent()
        search_tool = next(t for t in agent.tools if t.__name__ == "search_products_by_query")

        result = search_tool("nonexistent product")

        assert result["status"] == "success"
        assert result["products"] == []
        assert result["total_found"] == 0
        assert "No products found" in result["message"]

    @pytest.mark.asyncio
    async def test_search_error_handling(self, inventory_agent, mock_vector_store):
        """Test error handling in search operations."""
        # Make the search raise an exception
        mock_vector_store.search.side_effect = Exception("Database connection error")

        agent = inventory_agent._build_agent()
        search_tool = next(t for t in agent.tools if t.__name__ == "search_products_by_query")

        result = search_tool("test query")

        assert result["status"] == "error"
        assert "error_message" in result
        assert "Database connection error" in result["error_message"]

    @pytest.mark.asyncio
    async def test_run_method(self, inventory_agent):
        """Test the run method returns expected response format."""
        response = await inventory_agent.run("Find smart TVs", "test-session-123")

        assert isinstance(response, dict)
        assert "response" in response
        assert isinstance(response["response"], str)

    @pytest.mark.asyncio
    async def test_stream_method(self, inventory_agent):
        """Test the stream method yields expected events."""
        events = []
        async for event in inventory_agent.stream("Find wireless headphones", "test-session-456"):
            events.append(event)

        # Should have at least status and result events
        assert len(events) >= 2

        # Check event types
        event_types = [e.get("type") for e in events]
        assert "status" in event_types
        assert "result" in event_types

        # Verify status event
        status_event = next(e for e in events if e.get("type") == "status")
        assert "message" in status_event
        assert "Searching inventory" in status_event["message"]

        # Verify result event
        result_event = next(e for e in events if e.get("type") == "result")
        assert "content" in result_event

    @pytest.mark.asyncio
    async def test_handle_a2a_request(self, inventory_agent):
        """Test handling A2A protocol requests."""
        request_data = {
            "messages": [{"role": "user", "content": "What smart TVs do you have in stock?"}],
            "context": {"session_id": "test-a2a-session"},
        }

        response = await inventory_agent.handle_a2a_request(request_data)

        assert isinstance(response, dict)
        assert "messages" in response
        assert len(response["messages"]) > 0
        assert response["messages"][0]["role"] == "assistant"

    @pytest.mark.asyncio
    async def test_product_data_extraction(self, inventory_agent):
        """Test that product data is correctly extracted from search results."""
        # Test with nested metadata structure
        nested_result = {
            "id": "TEST-001",
            "metadata": {"name": "Test Product", "price": 99.99, "category": "Test Category", "stock_quantity": 10},
        }

        agent = inventory_agent._build_agent()
        search_tool = next(t for t in agent.tools if t.__name__ == "search_products_by_query")

        # Override mock to return nested structure
        inventory_agent._search_store.search.return_value = [nested_result]

        result = search_tool("test")

        assert result["status"] == "success"
        assert len(result["products"]) == 1
        product = result["products"][0]
        assert product["name"] == "Test Product"
        assert product["price"] == 99.99

    @pytest.mark.asyncio
    async def test_stream_with_tool_calls(self, inventory_agent, mock_agent_runner):
        """Test streaming with tool call events."""

        # Create a more complex mock event sequence
        async def mock_run_with_tools(*args, **kwargs):
            # Tool call event
            tool_event = Mock()
            tool_event.content = Mock()
            tool_event.content.parts = [Mock()]
            tool_event.content.parts[0].function_call = Mock()
            tool_event.content.parts[0].function_call.name = "search_products_by_query"
            tool_event.is_final_response = Mock(return_value=False)
            yield tool_event

            # Final response event
            final_event = Mock()
            final_event.content = Mock()
            final_event.content.parts = []
            final_event.content.text = "I found 2 smart TVs for you."
            final_event.is_final_response = Mock(return_value=True)
            yield final_event

        mock_agent_runner.run_async = mock_run_with_tools

        events = []
        async for event in inventory_agent.stream("Find TVs", "test-session"):
            events.append(event)

        # Should have tool call event
        tool_events = [e for e in events if e.get("type") == "tool_call"]
        assert len(tool_events) > 0
        assert tool_events[0]["tool_name"] == "search_products_by_query"

    @pytest.mark.asyncio
    async def test_concurrent_requests(self, inventory_agent):
        """Test that the agent can handle concurrent requests."""
        import asyncio

        # Create multiple concurrent requests
        tasks = []
        for i in range(5):
            task = inventory_agent.run(f"Find product {i}", f"session-{i}")
            tasks.append(task)

        # Run all tasks concurrently
        results = await asyncio.gather(*tasks)

        # Verify all requests completed successfully
        assert len(results) == 5
        for result in results:
            assert isinstance(result, dict)
            assert "response" in result

    def test_tool_descriptions(self, inventory_agent):
        """Test that all tools have proper descriptions."""
        agent = inventory_agent._build_agent()

        for tool in agent.tools:
            # Check that each tool has a docstring
            assert tool.__doc__ is not None
            assert len(tool.__doc__.strip()) > 0

            # Check specific tool descriptions
            if tool.__name__ == "check_product_availability":
                assert "product is available" in tool.__doc__.lower()
            elif tool.__name__ == "search_products_by_query":
                assert "search" in tool.__doc__.lower()
            elif tool.__name__ == "search_products_by_category":
                assert "category" in tool.__doc__.lower()
